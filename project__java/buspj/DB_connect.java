...
public class DB_connect {
    ...
    public void sing_db(Member new_member) {
        ...
        try {
            ...
            String sql="insert into new_table(id,pw,email,name,phone,mileage,used_mileage)";
            sql+= "values (?,?,?,?,?,?,?)";
    ...
    public String[] start() {
        ...
        try {
            ...
            String []add = new String[20000];
            Class.forName("com.mysql.cj.jdbc.Driver");
            conn = DriverManager.getConnection("jdbc:mysql:" +
                    "//localhost:3306/bus", "root", "1234");
            System.out.println("DB 연결 완료");

            String sql = "select distinct start_terminal from bongjunho";
            pstmt = conn.prepareStatement(sql);
            rs = pstmt.executeQuery();
            int i =0;
            while (rs.next()){
    ...
    //도착지 db
    public String[] end(String start) {
        Connection conn;
        ResultSet rs = null;
        Statement stmt = null;
        PreparedStatement pstmt = null;
        try {
            ...
            String sql = "select distinct end_terminal from bongjunho where start_terminal = '" + start + "'";
            pstmt = conn.prepareStatement(sql);
            rs = pstmt.executeQuery();
            int i = 0;
            while (rs.next()) {
    ...
    // 출발 시간 db
    public String[] time(String start, String end) {
        Connection conn;
        ResultSet rs = null;
        Statement stmt = null;
        PreparedStatement pstmt = null;
        try {
            ...
            String sql = "select distinct start_time from bongjunho where start_terminal = '" + start + "' and end_terminal = '" + end + "'";
            pstmt = conn.prepareStatement(sql);
            rs = pstmt.executeQuery();
            int i = 0;
            while (rs.next()) {
    ...
    // 표 DB
    public Ticket ticket_load(String start, String end, String date, String starttime) {
        Connection conn;
        ResultSet rs = null;
        Ticket t = new Ticket();
        String SQL = "SELECT end_time, fee FROM bongjunho WHERE start_terminal = '" + start + "' and end_terminal = '" + end + "' and start_time = '" + starttime + "'";
        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
            conn =
                    DriverManager.getConnection("jdbc:mysql:" +
...
//            pstmt.setString(0, end);
            ...
            while (rs.next()) {
                String end_time = rs.getString(1);
                int price = rs.getInt(2);

                t.insertTicket(price, end_time);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    // 회원 예매 정보 DB에 저장
    public void saveUserReservation(String id, String start, String end, String date, String starttime, int price, int seatNum) {
        Connection conn;
        Statement stmt = null;
        PreparedStatement pstmt = null;
        try {
            ...
            String sql="insert into reservation_user(id,start,end,date,starttime,price,seatnum)";
            sql += "values (?,?,?,?,?,?,?)";
            pstmt = conn.prepareStatement(sql);
            ...
            pstmt.setInt(6, savePrice);
            pstmt.setInt(7,saveSeatnum);
    ...
    public CheckUp loadUserReservation(String id) {
        ...
        String SQL = "SELECT date, start, end, starttime, price, seatnum FROM reservation_user where id='"+ id +"'";
        ...
        try {
            ...
            while (rs.next()) {
                ...
                String seat = String.valueOf(rs.getInt(6));
                c.insertData(date,start,end,starttime, price, seat);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public String[] load_information(String id) {
        ...
        try {
            ...
            String sql = "select start,end,date,starttime,price from reservation_user where id = '" + id + "'";
            pstmt = conn.prepareStatement(sql);
            rs = pstmt.executeQuery(sql);
    ...
    // 적용한 마일리지 불러오기
    public int used_mileage_get(String id) {
        Connection conn;
        Statement stmt = null;
        ResultSet rs = null;
        PreparedStatement pstmt = null;
        ...
        try {
            ...
            String sql="select used_mileage from new_table where id=?";
            pstmt = conn.prepareStatement(sql);
            pstmt.setString(1, id);
            rs = pstmt.executeQuery();
            while (rs.next()) {
                used_mileage=rs.getInt(1);
            }
            pstmt.executeUpdate();
            pstmt.close();
            return used_mileage;
    ...
    // 사용한 마일리지 업데이트
    public void set_used_mileage(String id, int mileage) {
        Connection conn;
        Statement stmt = null;
        PreparedStatement pstmt = null;
        try {
            ...
            String sql="update new_table set used_mileage="+ mileage + " where id='" + id + "'";
            pstmt = conn.prepareStatement(sql);
