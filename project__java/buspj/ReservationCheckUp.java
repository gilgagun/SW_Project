...
class CheckUp {
    ...
    public void insertData(String date, String start, String end, String starttime, String price, String seat) {
        this.date.add(date);
...
public class ReservationCheckUp extends JFrame implements MouseListener {
    ...
    String[] colName = {"출발날짜", "출발 터미널", "도착 터미널", "출발 시간", "가격", "좌석"};   // 테이블 열 이름
    CheckUp c;
    ...
    public ReservationCheckUp(String id) {
        ...
        for (int i = 0; i < c.date.size(); i++) {
            String[] data = {c.date.get(i), c.start.get(i), c.end.get(i), c.starttime.get(i), c.price.get(i), c.seat.get(i)};
            model.addRow(data);
        }
        ...
        bt_del.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent e) {
                ...
                if (answer == JOptionPane.YES_OPTION) {
                    ...
                    int used_point = DB.used_mileage_get(id);
                    point -= 10;
                    point += used_point; // 마일리지로 결제한 게 있다면 그거 다시 돌려줌
                    DB.set_mileage(id, point);       // 마이너스 된 마일리지 업데이트

                    used_point -= used_point;        // 적용했던 마일리지 취소하기
                    DB.set_used_mileage(id, used_point);  // 적용했던 마일리지 업데이트
...
//                    DB.plus_seats(arr);             // 삭제한 표의 좌석 정보 업데이트
    ...
    @Override
    public void mouseClicked(MouseEvent e) {
        String[] info = new String[6];     // 추출한 정보를 담을 배열
