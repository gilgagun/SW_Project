...
//    JLabel personnel; // 인원을 담을 JLabel
    ...
    public SeatsCenter(SeatsSelect frame, String id, String start, String end, String date, String starttime) {
        setLayout(null);
        this.id = id;   // 회원 아이디 정보 저장
        Ticket t = DB.ticket_load(start, end, date, starttime);
        this.price = t.getPrice();   // 해당 표의 가격 가져오기
        ...
        payment.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent e) {
                if (col == 10000) {
                    ...
                    new Payment(frame, id, start, end, date, starttime, price, seatNum[row][col]);
                    frame.setVisible(false);
                }
            }
        });
...
public class SeatsSelect extends JFrame {
    public SeatsSelect(ReservationMain frame, String id, String start, String end, String date, String starttime) {
        setTitle("버스타슈~");
        setSize(1000,800);
        setResizable(false);
        ...
        mainContainer.add(new SeatsCenter(this, id, start, end, date, starttime), BorderLayout.CENTER);
